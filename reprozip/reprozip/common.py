...
def load_files(config, runs, key):
    """Loads the input_files and output_files part of the configuration.

    This tests for duplicates, merge the lists of executions, and optionally
    loads from the runs for reprozip < 0.7 compatibility.
    """
    files_list = config.get(key) or []
    ...
    files = {}
    paths = {}
    expected_keys = set(['name', 'path', 'executions'])
    for f in files_list:
        if set(f) != expected_keys:
            raise InvalidConfig("File has invalid keys; expected name, path,"
                                "executions")
        name, path, executions = f['name'], f['path'], f['executions']
        path = PosixPath(path)
        if path in paths:
            if paths[path] == name:
                ...
                name = paths[name]
            files[name][1] = sorted(set(files[name][1]) | set(executions))
        else:
            paths[path] = name
            files[name] = path, sorted(set(executions))
...
def load_config(filename, canonical, File=File, Package=Package):
    ...
    input_files = load_files(config, runs, 'input_files')
    output_files = load_files(config, runs, 'output_files')
...
# The name field is the identifier the user will use to access these files.""")
        ...
        for section, data in (('input_files', input_files),
                              ('output_files', output_files)):
            fp.write("\n%s:\n" % section)
            for n, (p, e) in iteritems(data):
                fp.write("""\
- name: {name}
  path: {path}
  executions: {execs}
""".format(name=n, path=unicode(p), execs=sorted(e)))
